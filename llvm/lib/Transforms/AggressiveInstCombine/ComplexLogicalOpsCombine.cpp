...
static cl::opt<unsigned> MaxLogicOpLeafsToScan(
    "aggressive-instcombine-max-logic-op-leafs", cl::init(8), cl::Hidden,
    cl::desc("Max leafs of logic ops to scan for aggressive instcombine."));

static cl::opt<unsigned> MaxDepthLogicOpsToScan(
    "aggressive-instcombine-max-depth-logic-ops", cl::init(8), cl::Hidden,
    cl::desc("Max depth of logic ops to scan for aggressive instcombine."));
...
void LogicalOpNode::printAndChain(raw_ostream &OS, uint64_t Mask) const {
  ...
  for (unsigned I = 1; I < MulElementCnt; I++) {
    MaskIdx = llvm::countTrailingZeros(Mask);
